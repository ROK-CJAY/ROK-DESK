...
export function YgoJudgeTablet() {
  ...
  const guide = useTabletGuide("ygo");
  ...
  return (
    <div className="flex h-dvh flex-col bg-bg text-fg" data-game={desk.gameId}>
      <header className="shrink-0 border-b border-border px-3 py-2.5 sm:px-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="min-w-0">
            <p className="font-mono text-[0.62rem] tracking-[0.2em] text-muted uppercase">ROK · YGO judge</p>
      ...
      <div className="min-h-0 flex-1 overflow-auto p-3">
        ...
        <div className="mt-3">
          <CardLookup catalog="ygo" formatName={desk.formatName} />
        </div>
      </div>
      <TabletGuide kind="ygo" open={guide.open} onClose={guide.close} />
    </div>
  );
}
...
function YgoSeat({
  ...
  return (
    ...
    >
      ...
      <p className="truncate text-sm text-muted">{player.archetype || player.extra || extraLabel}</p>

      <div className="mt-3">
        <div className={cn("mb-1.5 flex items-center justify-between", rtl && "lg:flex-row-reverse")}>
          <span className="text-xs text-muted">Life points</span>
          <span className={cn("font-display text-3xl font-semibold tabular-nums", out ? "text-live" : "text-fg")}>
            {player.resource}
          </span>
        </div>
        <DeltaPad onDelta={onLp} defaultAmount={100} max={16000} size="tablet" className={cn(rtl && "lg:justify-end")} />
      </div>
