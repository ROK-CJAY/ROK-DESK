...
export function SwuJudgeTablet() {
  ...
  const guide = useTabletGuide("swu");
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-bg text-fg" data-game={desk.gameId}>
      <header className="shrink-0 border-b border-border px-3 py-2.5 sm:px-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="min-w-0">
            <p className="font-mono text-[0.62rem] tracking-[0.2em] text-muted uppercase">ROK · SWU judge</p>
      ...
      <div className="flex-1 p-3">
        ...
        <div className="mt-3">
          <CardLookup catalog="swu" />
        </div>
      </div>
      <TabletGuide kind="swu" open={guide.open} onClose={guide.close} />
    </div>
  );
}
    ...
    >
      ...
      <div className="mt-3">
        <div className={cn("mb-1.5 flex items-center justify-between", rtl && "lg:flex-row-reverse")}>
          <span className="text-xs text-muted">Base HP</span>
          <span className={cn("font-display text-3xl font-semibold tabular-nums", out ? "text-live" : "text-fg")}>
            {player.resource}
          </span>
        </div>
        <DeltaPad onDelta={onHp} size="tablet" className={cn(rtl && "lg:justify-end")} />
      </div>
