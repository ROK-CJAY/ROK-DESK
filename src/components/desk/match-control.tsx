    ...
    <section className="rounded-xl border border-border bg-surface p-4 shadow-panel sm:p-5">
      ...
      {commanderTable ? (
        ...
        <div className="space-y-4">
          <div className="flex flex-col items-center rounded-lg bg-surface-2 px-4 py-3">
            <div className="font-display flex items-center gap-3 text-5xl leading-none font-semibold tabular-nums">
            ...
            <p className="mt-1 font-mono text-[0.6rem] tracking-[0.16em] text-muted uppercase">
...
function PlayerColumn({
  ...
  return (
    <div className={cn("min-w-0 rounded-lg bg-surface-2 p-4", rtl && "lg:text-right")} data-game={desk.gameId}>
      <div className={cn("mb-3 flex items-center justify-between gap-2", rtl && "flex-row-reverse")}>
        <p className="font-mono text-[0.65rem] tracking-[0.2em] text-muted uppercase">
          {side === "p1" ? "Player 1" : "Player 2"}
        </p>
        <div className={cn("flex gap-1.5", rtl && "flex-row-reverse")}>
          <Button
            variant={desk.gameWinnerSide === side ? "live" : "secondary"}
            size="sm"
            onClick={() => (desk.gameWinnerSide === side ? clearWinners() : gameWin(side))}
          >
            Game
          </Button>
          <Button
            variant={desk.winnerSide === side ? "live" : "outline"}
            size="sm"
            onClick={() => {
        ...
        {game.resource.pipStyle === "team" ? (
          <div className={cn("flex flex-wrap", rtl && "lg:justify-end")}>
            <TeamPips
              ...
              tone="desk"
              onToggle={(index) => {
                const down = toggleMonDown(player.down, index, max);
                onChange({ down, resource: remainingFromDown(down, max) });
              }}
            />
          </div>
        ) : game.resource.pips ? (
